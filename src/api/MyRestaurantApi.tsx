...
export const useUpdateMyRestaurantOrder = () => {
  ...
  const updateMyRestaurantOrderRequest = async (
    updateStatusOrederRequest: UpdateOrderStatusRequest
  ) => {
    const accessToken = await getAccessTokenSilently();

    const response = await fetch(
      `${API_BASE_URL}/api/my/restaurant/order/${updateStatusOrederRequest.orderId}/status`,
      {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${accessToken}`,
        ...
        body: JSON.stringify({ status: updateStatusOrederRequest.status }),
      }
    );
